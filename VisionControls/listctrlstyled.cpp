...
#define INCLUDE_LISTCTRLSTYLED __declspec(dllexport)
...
COLORREF CListCtrlStyled::GetRowBgColor(int nRow) {
    ...
    case CDDS_ITEMPOSTPAINT:
        {
			...
			{	m_item_selected = false;
				...
				//CPen * old_pen = pDC->SelectObject(&pen); //Tell the DC to use this pen from now on.
				//CRect rect;
				//GetItemRect(nItem, rect, LVIR_BOUNDS);
				//pDC->SelectStockObject(NULL_BRUSH); // set NULL_BRUSH
...
void CListCtrlStyled::MergeLogFont(LOGFONT * font,LOGFONT * dest)
{
	...
	*/
	...
	if(wcscmp(font->lfFaceName,this->m_Default_LOGFONT.lfFaceName) != 0)
		wcscpy_s(dest->lfFaceName,font->lfFaceName);
